...
namespace TorneusClienteWeb.Servicios_de_Datos
{
    public class NotificacionServicioDatos
    {
        ...
        public async Task<List<NotificacionDTO>> ObtenerListadoNotificacionesOrganizador(UsuarioLogueado usuario)
        {
            try
            {
                _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

                var response = await _httpClient.PostAsJsonAsync($"api/Notificacion/ObtenerNotificacionesOrganizador", usuario);
